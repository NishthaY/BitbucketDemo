...
class Coverage_tier extends ColumnValidation {
    ...
    function validate( $has_headers ) {
        ...
        // we will pull that data from a different column.  This version of the validate
        ...
        // data from the parent.  Furthermore, if we are missing parent we will pull from
        // a grandparent.
        // ------------------------------------------------------------------
        ...
        $p_fh               = null;
        $parent_column_name = "plan";
        $parent_display     = $this->get_column_display($parent_column_name);

        $gp_fh                      = null;
        $grandparent_column_name    = "plan_type";
        $grandparent_display        = $this->get_column_display($grandparent_column_name);
        ...
        try{
            ...
            $p_url = $this->get_validation_filename($this->identifier, $this->identifier_type, $parent_column_name);    // Grab the parent file, if we can.
            $gp_url = $this->get_validation_filename($this->identifier, $this->identifier_type, $grandparent_column_name);    // Grab the grandparent file, if we can.
            ...
            if ($fh) {
                ...
                if ( file_exists($gp_url) ) $gp_fh = fopen($gp_url, "r");

                $index = 0;
                while (($line = fgets($fh)) !== false)
                {
                    // PARENT: Read the corresponding parent line too.
                    $p_line = "";
                    if ( $p_fh ) {
                        $p_line = fgets($p_fh);
                        $p_line = trim($p_line);
                    ...
                    // GRANDPARENT: Read the corresponding grandparent line too.
                    $gp_line = "";
                    if ( $gp_fh ) {
                        $gp_line = fgets($gp_fh);
                        $gp_line = trim($gp_line);
                        $gp_line = getStringValue($gp_line);
                    }
                    ...
                    if ( $line == "" ) $line = getStringValue($gp_line);
                    ...
                    if ( $this->validation_required )
                    {
                        ...
                        if ( $line == "" )
                        {
                            $message = "Row {$index} for the ".strtolower($this->column_display)." column is invalid.  This field was empty and we tried to infer it from first the {$parent_display} column then the {$grandparent_display} column, but both where empty too.";
            ...
            // GRANDPARENT: close the parent file.
            if ( $gp_fh != null ) fclose($fh);
